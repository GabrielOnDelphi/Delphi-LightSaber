...
var
  Trans: TTranslator;
  LangFolder, TestFile: string;
begin
  Trans:= TTranslator.Create(AppDataCore);
  TRY
    LangFolder:= Trans.GetLangFolder;
    TestFile:= LangFolder + 'TestSave.ini';

    // Ensure Lang folder doesn't exist (to test that SaveTranslation creates it)
    if FileExists(TestFile)
    then DeleteFile(TestFile);
    if DirectoryExists(LangFolder)
    then RemoveDir(LangFolder);

    // SaveTranslation should create the Lang folder
    Trans.SaveTranslation(TestFile, TRUE);

    Assert.IsTrue(DirectoryExists(LangFolder), 'SaveTranslation should create Lang folder');
    Assert.IsTrue(FileExists(TestFile), 'SaveTranslation should create the INI file');
