...
-----------------------------------------------------------------------------------------
   ...
   **Features**
     ...
     User perspective
        ...
        See 'README AutoTranslator.md' for translation tips and documentation.
...
USES
   LightCore, LightCore.IO,
   LightVcl.Common.IniFile, LightVcl.Common.VclUtils;
...
begin
  if Assigned(FAppData)
  then WriteString('Last_Language', CurLanguageName);
  inherited Destroy;
end;
...
begin
  ...
  OR (HasAct AND ParseCtrlsWithAction) then
   begin
    ...
    // Translate Caption for controls, but not for TForm (form title bar may contain version info etc.)
    if NOT Component.InheritsFrom(TCustomForm)
    then ReadProperty(Component, '', 'Caption', Section, Ini);
 ...
 if Component.Tag = DontTranslate then EXIT;
  ...
  OR (HasAct AND ParseCtrlsWithAction) then
   begin
    ...
    // Save Caption for controls, but not for TForm (form title bar may contain version info etc.)
    if NOT Component.InheritsFrom(TCustomForm)
    then WriteProperty(Component, '', 'Caption', Section, Ini);
