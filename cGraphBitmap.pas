...
USES
   System.Types, Vcl.ExtCtrls, System.SysUtils, Vcl.Graphics, System.Classes, cGraphFX;
...
USES
   ccCore, cmMath, cGraphLoader.Resolution;
 ...
 EXCEPT
   On EOutOfMemory DO
      ...
      MesajWarning('Cannot set bitmap size to: '+ IntToStr(Width)+ ' x '+ IntToStr(Height)+ 'pixels.'
             +CRLF+'Required RAM: '+ FormatBytes(RequiredRam, 2));
 END;
end;
...
begin
 Assert(BMP <> NIL);
 Result:= (NewWidth * NewHeight) * (cGraphLoader.Resolution.GetBitsPerPixel(BMP) DIV 8);
end;
