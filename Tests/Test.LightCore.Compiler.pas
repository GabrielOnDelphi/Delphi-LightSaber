unit Test.LightCore.Compiler;

{=============================================================================================================
   Unit tests for LightCore.Compiler.pas
...
type
  ...
  public
    ...
    [Test]
    [TestCase('Delphi_Yukon',  '')]
    [TestCase('Delphi_Athens', '')]
    [TestCase('Delphi_Alex',   '')]
    [TestCase('Delphi_Sydney', '')]
    [TestCase('Delphi_Rio',    '')]
...
begin
  ...
  Assert.IsTrue(Delphi_Yukon > 0,  'Delphi_Yukon must be defined');
  Assert.IsTrue(Delphi_Athens > 0, 'Delphi_Athens must be defined');
  Assert.IsTrue(Delphi_Alex > 0,   'Delphi_Alex must be defined');
  Assert.IsTrue(Delphi_Sydney > 0, 'Delphi_Sydney must be defined');
  Assert.IsTrue(Delphi_Rio > 0,    'Delphi_Rio must be defined');
end;
...
begin
  ...
  Assert.IsTrue(Delphi_Tokyo > 0,   'Delphi_Tokyo must be defined');
  Assert.IsTrue(Delphi_Berlin > 0,  'Delphi_Berlin must be defined');
  Assert.IsTrue(Delphi_Seattle > 0, 'Delphi_Seattle must be defined');
...
begin
  ...
  Assert.AreEqual(37.0, Delphi_Yukon, 0.001, 'Delphi 13 Yukon should be 37.0');
end;
...
begin
  ...
  Assert.AreEqual(36.0, Delphi_Athens, 0.001, 'Delphi 12 Athens should be 36.0');
end;
...
begin
  ...
  Assert.AreEqual(35.0, Delphi_Alex, 0.001, 'Delphi 11 Alexandria should be 35.0');
end;
...
begin
  ...
  Assert.AreEqual(34.0, Delphi_Sydney, 0.001, 'Delphi 10.4 Sydney should be 34.0');
end;
...
begin
  ...
  Assert.AreEqual(33.0, Delphi_Rio, 0.001, 'Delphi 10.3 Rio should be 33.0');
end;
...
begin
  ...
  Assert.AreEqual(32.0, Delphi_Tokyo, 0.001, 'Delphi 10.2 Tokyo should be 32.0');
end;
...
begin
  ...
  Assert.IsTrue(Delphi_Yukon > Delphi_Athens,   'Yukon should be newer than Athens');
  Assert.IsTrue(Delphi_Athens > Delphi_Alex,    'Athens should be newer than Alex');
  Assert.IsTrue(Delphi_Alex > Delphi_Sydney,    'Alex should be newer than Sydney');
  Assert.IsTrue(Delphi_Sydney > Delphi_Rio,     'Sydney should be newer than Rio');
  Assert.IsTrue(Delphi_Rio > Delphi_Tokyo,      'Rio should be newer than Tokyo');
  Assert.IsTrue(Delphi_Tokyo > Delphi_Berlin,   'Tokyo should be newer than Berlin');
  Assert.IsTrue(Delphi_Berlin > Delphi_Seattle, 'Berlin should be newer than Seattle');
  Assert.IsTrue(Delphi_Seattle > Delphi_XE8,    'Seattle should be newer than XE8');
...
begin
  ...
  Assert.AreNotEqual(Delphi_Yukon, Delphi_Athens, 0.001);
  Assert.AreNotEqual(Delphi_Athens, Delphi_Alex, 0.001);
  Assert.AreNotEqual(Delphi_Alex, Delphi_Sydney, 0.001);
  Assert.AreNotEqual(Delphi_Sydney, Delphi_Rio, 0.001);
  Assert.AreNotEqual(Delphi_Rio, Delphi_Tokyo, 0.001);
end;
...
begin
  ...
  Assert.IsTrue(RTLVersion >= Delphi_Rio,
    Format('RTLVersion %.1f should be >= Delphi Rio (%.1f)', [RTLVersion, Delphi_Rio]));
end;
...
begin
  ...
  Assert.IsTrue(
    (RTLVersion >= Delphi_7) AND (RTLVersion <= Delphi_Yukon + 1),
...
begin
  ...
  Assert.AreEqual(Delphi_Athens, Delphi_Athenes, 0.001, 'Delphi_Athenes should alias Delphi_Athens');
  Assert.AreEqual(Delphi_Alex, Delphi_Alexa, 0.001, 'Delphi_Alexa should alias Delphi_Alex');
end;
...
begin
  ...
  Assert.IsTrue(Delphi_Yukon - Delphi_Athens >= 0.5, 'Version gap should be significant');
  Assert.IsTrue(Delphi_Athens - Delphi_Alex >= 0.5, 'Version gap should be significant');
