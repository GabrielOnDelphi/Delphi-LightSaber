...
USES
  ciDownload, cmDebugger, ccAppdata; //, FormUpdaterNotifier;
  ...
  begin
   ...
   if NOT Result then
    begin
     ...
     if ShowConnectFail
     then MesajError('The updater file seems to be invalid. CRLF File: '+ UpdaterFileLocation);
...
procedure TUpdater.SaveTo(FileName: string);
begin
...
procedure TUpdater.LoadFrom(FileName: string);
begin
 VAR IniFile := TCubicIniFile.Create('Updater');
 try
   { Internal state}
