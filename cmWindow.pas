...
begin
  ...
  WHILE NextHandle > 0 DO
   begin
     ...
     s:= StrPas(NextTitle);
     if ccCore.find(WindowTitle, s, PartialSearch, CaseSens)
     then
      begin
       Result:= NextHandle;
       Exit;
...
{-------------------------------------------------------------------------------------------------------------
  ...
  Window := cmWindow.FindTopWindowByClass(ctAppWinClassName);
  if Window = 0
...
VAR Wnd: HWND;
begin
 Assert(ClassName > '', 'AppWinClassName was not defined!');
