...
3. ~~**The queued `WM_POSTINIT` dies with the handle.**~~ **FIXED 2026-08-20 at the source** — the post-init step is no longer a posted window message. `TAppData.CreateMainForm` calls `TLightForm.SchedulePostInitialize`, which uses `TThread.ForceQueue`; that entry lives in the RTL queue, which no window owns, so no handle recreation can lose it (drained by `CheckSynchronize` from both `TApplication.WndProc` `WM_NULL` and `TApplication.Idle`). What used to happen: `CreateMainForm` posted `WM_POSTINIT` to the main form, and with `Application.ShowMainForm := FALSE` the form was invisible, so the broadcast took the `SendMessage` branch and the handle died **synchronously** — before the loop ever pumped. `FormPostInitialize` never fired and everything it was supposed to initialize stayed NIL. **Consequences 1 and 2 above are NOT fixed, so the guard stays.**
