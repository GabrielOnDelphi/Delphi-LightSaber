...
procedure TLightStream.WriteShortInt(s: ShortInt);
begin
  WriteBuffer(s, 1);
end;
...
procedure TLightStream.WriteSmallInt(s: SmallInt);
begin
  WriteBuffer(s, 2);
end;
...
function TLightStream.AsBytes: TBytes;
begin
  Position:= 0;
...
procedure TLightStream.CheckSafetyLimit(Count: Cardinal);
begin
  if (Count+ Position > Size)
  then RAISE Exception.Create('String too large. Trying to read: '+ IntToStr(Count)+ ' bytes');
end;
