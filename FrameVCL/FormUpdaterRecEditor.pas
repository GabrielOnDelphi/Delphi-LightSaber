...
begin
  ...
  then Form.LoadFromFile(GetNewsFileName);     { Silent load — no file-open dialog }
...
begin
  if NOT StringToClipboard(GetNewsFileName)
  then MessageWarning('Could not copy to clipboard.');
end;
...
{ Load a news file silently (no file-open dialog). Used by CreateFormModal to pre-populate
  the editor with the existing OnlineNews.ini when one is present. }
procedure TfrmRecEditor.LoadFromFile(const FileName: string);
VAR
  News: RNews;
begin
  FName:= FileName;
  if NOT News.LoadFrom(FName) then
   begin
    MessageError('Cannot load: not a valid LightUpdater news file (signature/version mismatch).');
    EXIT;
   end;
