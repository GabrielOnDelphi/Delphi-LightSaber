...
USES
  Winapi.Windows,
  LightCore.IO, LightCore.Types, LightCore.StreamBuff;
...
--------------------------------------------------------------------------------------------------}
procedure TfrmTestIO.btnStreamWriteClick(Sender: TObject);
VAR
   TSL: TStringList;
   Stream: TLightStream;
   dDate: TDate;
...
begin
  Memo.Clear;
  Memo.Lines.Add('Writing TLightStream.bin ...');
  ...
  TRY
   ...
   Stream.WriteStringA('Test ANSI with special chars: ���');
  ...
  Memo.Lines.Add('Write successful - ' + IntToStr(GetTickCount));
end;



procedure TfrmTestIO.btnStreamReadClick(Sender: TObject);
VAR
   TSL: TStringList;
   Stream: TLightStream;
   dDate: TDate;
...
begin
  ...
  begin
    Memo.Lines.Add('Error: TLightStream.bin not found - click "Write" first.');
    EXIT;
  end;

  dDate:= EncodeDate(2024, 08, 10);
  Memo.Lines.Add('Reading TLightStream.bin ...');
  ...
  TRY
   ...
   // Test alternative header reading (returns Word version, signature-only)
   ...
   if Pos('���', string(TestAnsi)) = 0 then RAISE Exception.Create('ReadStringA special chars failure!');
   TestAnsi := Stream.ReadStringA;
  ...
  Memo.Lines.Add('Read successful - All tests passed! - ' + IntToStr(GetTickCount));
end;
