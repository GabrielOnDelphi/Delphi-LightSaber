    ...
    begin
     ...
     then
      begin
        ...
        then
         begin
          Tag:= ccCore.CopyTo(Result, iStart1, iEnd);
  ...
  begin
   ...
   if QStart> 0 then
    begin
     ...
     if QEnd> 0 then
      begin
        Result:= ccCore.CopyTo(HtmlTag, QStart+1, QEnd-1);               { +1 -1 to remove the quotes }
        {
        if Pos('?', Result) > 0
  ...
  begin
   ...
   if QStart> 0 then
    begin
     ...
     if QEnd> 0
     then Result:= ccCore.CopyTo(HtmlTag, QStart+1, QEnd-1);               { +1 -1 to remove the quotes }
    end;
  end;
end;
 ...
 REPEAT
   ...
   then
    begin
      ...
      then
       begin
        Tag:= ccCore.CopyTo(HtmlBody, Tag1End, Tag2Start-1);
        Result.Add(Tag);
        Offset:= Tag1End;
       end
      else Break;
    ...
    begin
     ...
     then
      begin
        ...
        then
         begin
          Tag:= ccCore.CopyTo(HtmlBody, iStart1, iEnd);
          if Tag = ''
    ...
    for cnt:= (StartPos+ Pad) to length(Text) DO
      ...
      then
       begin
         ...
         { Add it to list }
         Inc(Result);
         AdreseExtrase.Add(ccCore.CopyTo(Text, StartPos, EndPos) );
