...
--------------------------------------------------------------------------------------------------------------
   HOW TO USE IT?
      ...
      Full demos here: c:\Projects\LightSaber\Demo\FMX\Template - Minimal app\FMX_MinimalApp.dpr
...
--------------------------------------------------------------------------------------------------------------
   ...
   DETAILS
     ...
     More about this here:
        c:\Projects\LightSaber\FrameFMX\FMX form event order.txt


     FormPreRelease

        TLightForm.FormPreRelease, unlike the other events, is guaranteed to run exactly once when the form is closed.
        This is the place for your clean-up code.
     ...
     Embedded forms
         ...
         This sets FEmbedded and AutoState=asNone BEFORE the inherited constructor runs.
         ...
         TThread.ForceQueue(NIL, ...) on Android:
           ForceQueue(NIL, ...) is reliable as long as the queuing code does not block the main thread before the message loop resumes.
           The Android FMX loop drains the sync queue (CheckSynchronize) on every pump, and TThread.ForceQueue has no platform-specific branch — verified in System.Classes.pas / FMX.Platform.Android.pas.
           One unverified report (delphipraxis.net topic 14705, Delphi 12.3) describes ForceQueue callbacks not firing on Android; it was never reproduced and never root-caused — an Embarcadero MVP on that thread attributed it to the main thread being overloaded by a long synchronous run ("Skipped frames" in logcat), not to a ForceQueue defect.
           So: don't do heavy synchronous work between the ForceQueue call and the loop resuming.
           Passing NIL (not a TThread instance) as the first parameter is required — a TThread arg makes the entry get dropped when that thread is freed.
...
TYPE
  { System.Messaging only names this type TMessageSubscriptionId from Delphi 12 Athens on.
    Before that, TMessageManager.SubscribeToMessage returned a plain Integer, and the width changed with the rename (Integer -> Int64), so an older compiler cannot just be handed the new name.
    Use TSubscriptionId everywhere in this unit instead of the RTL name. }
  ...
  protected
    ...
    { Called by FormKeyUp when the user presses Back (Android) or Escape (desktop).
      ...
      Return TRUE if handled. Return FALSE to let the default proceed (moveTaskToBack / Close). }
  ...
  public
    ...
    { Embedded equivalent of FormCloseQuery.
      Returns TRUE if this form can be closed/navigated away from.
    ...
    { Android only: reserve the system status-bar and navigation-bar safe-area as form Padding so child controls (Align=Top/Bottom/Client) flow naturally inside the visible window instead of drawing under the clock/battery (top) or the gesture pill / navigation buttons (bottom).
      Necessary because Android 16 (targetSdkVersion=36) ignores `windowOptOutEdgeToEdgeEnforcement` — see https://developer.android.com/about/versions/16/behavior-changes-16.
      Called automatically from AfterConstruction, so every TLightForm-descended form (main + secondaries) gets it for free.
      No-op on non-Android platforms (Windows has native window decorations, iOS handles safe-area via SystemStatusBar.Visibility). }
...
{-------------------------------------------------------------------------------------------------------------
   SHUTDOWN SENTINEL
   GAppShuttingDown flips TRUE the moment FMX/OS signals app teardown, so the destructor assert (TLightForm.Destroy) can tell legitimate shutdown — where Application.DestroyComponents frees a still-visible secondary form — apart from the FreeAndNil-after-ShowModal antipattern it guards.
   Necessary because at that instant NONE of Application.Terminated, IFMXApplicationService.Terminating, or csDestroying(Application) is set yet: the OS Destroy path fires only TApplicationEvent.WillTerminate then Halt -> DoneApplication -> DestroyComponents (in Androidapi.AppGlue.pas, FMX.Platform.Android.pas and FMX.Forms.pas).
   We subscribe to BOTH the WillTerminate event (OS destroy) and TApplicationTerminatingMessage (programmatic Application.Terminate), so both teardown paths set the flag.
-------------------------------------------------------------------------------------------------------------}
VAR
...
begin
  { AutoState MUST be set BEFORE inherited Create — same reason as in CreateEmbedded below: the inherited chain streams the .fmx and fires Loaded before returning.
    Loaded's 'if AutoState = asUndefined then AutoState:= AppData.GetAutoState(Self)' would otherwise still see asUndefined and raise 'Form was not created via AppData.CreateForm()!' for every direct call of this constructor (there is no pending-queue entry for direct creation). }
...
{ FEmbedded and AutoState MUST be set BEFORE inherited Create.
  Reason: FMX's TCommonCustomForm.Create reads the .fmx resource inside the inherited chain and fires Loaded before returning.
  Loaded checks `if AutoState = asUndefined then AutoState:= AppData.GetAutoState(Self)`, and GetAutoState raises if the class is not in the pending queue (which it isn't for CreateEmbedded — we bypass AppData.CreateForm's queueing).
  Pre-setting AutoState=asNone makes Loaded skip the lookup.

  FEmbedded is set early so AfterConstruction (fired by the runtime AFTER the entire constructor chain returns) sees TRUE and skips its auto-Show. }
...
{ Marks this form as embedded and reparents ALayout (e.g. layRoot) into AParent (e.g. a TTabItem).
  ...
  Embedded forms must use asNone, because SaveForm writes position and size, which is meaningless for embedded layouts. }
...
begin
  ...
  { Ensure the form is reachable.
    ...
    Every other run: the position comes from the INI and is normally correct — but it was saved under whatever monitor layout existed at the time.
    Unplug a second monitor (or plug into a smaller one) and the restored Left/Top can land entirely outside the remaining desktop.
    The app then runs perfectly with a window nobody can see, which reads as a freeze; the tray icon still toggles a window that is drawn off-screen.
    Clamp only when the rectangle intersects no display at all, so a form deliberately parked on a secondary monitor is left alone. }
  ...
  { Android: reserve system-bar safe-area. Two complementary mechanisms:
      1. Synchronous call (here): if the activity's onApplyWindowInsets has ALREADY fired by the time this form is constructed (any form opened after the app's first layout — every secondary form, and the main form on a warm restart), the insets are known, so this captures them BEFORE the first paint and there is no flash of edge-to-edge.
      2. RegisterInsetsListener (below): on a COLD start the very first form constructs BEFORE Android dispatches its first onApplyWindowInsets — that dispatch runs after onResume + the first layout traversal, which is strictly after Pascal construction (Android lifecycle contract). At this point isWindowInsetsDefined is FALSE and the synchronous call above necessarily early-exits. Rather than poll-and-guess how many message-loop pumps until the dispatch lands (it is device-dependent — 1 on a OnePlus Nord, more on a Samsung Tab S11 Ultra), we subscribe to the callback FMX already fires from inside its own decor-view listener (FMXNativeActivity -> onActivityInsetsChangedListener.insetsChanged). It fires exactly when insets become available and again on every later change (rotation, multi-window resize), re-applying padding to every live TLightForm. Event-driven, no polling. See RegisterInsetsListener + TInsetsChangedListener.
  ...
  { # Hardware Back / Esc }
  { Descendant .fmx files are streamed as `object` (not `inherited`), so they do NOT inherit OnKeyUp=FormKeyUp from LightForm.fmx — each loads only its own resource by class name.
    Without wiring it here the Back/Esc ladder in FormKeyUp never runs, and FMX's default KeyUp closes the active form (and on the main form that calls Application.Terminate).
    NOT Assigned guards a form that wired its own OnKeyUp. }
...
begin
  ...
  { Mark initialization complete so saveBeforeExit can save form state.
    ...
    Defer via ForceQueue: when several forms are queued (CreateMainForm + CreateForm), FMX's RealCreateForms constructs them sequentially and each fires Loaded.
    Calling EndInitialization synchronously here flips Initializing=FALSE on the FIRST form, so any LoadForm side-effect raise in a LATER form would let saveBeforeExit persist half-constructed state.
    Posting to the message queue defers the flag-flip until RealCreateForms has returned and all queued forms are fully loaded.
    EndInitialization is idempotent (just sets a Boolean), so multiple posts are harmless.

    FormPostInitialize runs in the SAME queued callback, AFTER EndInitialization, so user startup code sees Initializing=FALSE and runs only once the message loop is pumping (window already painted).
    One queued block per TLightForm — FIFO ordering keeps EndInitialization before FormPostInitialize.
    This is the deferral the VCL frame gets from PostMessage(WM_POSTINIT); FMX forms have no HWND, so ForceQueue is the cross-platform equivalent.

    NAMED method (TThreadMethod overload), NOT an anonymous block: Destroy cancels the pending entry via TThread.RemoveQueuedEvents(QueuedPostInitialize), which matches on Code+Data.
    An anonymous TThreadProcedure capturing Self cannot be removed — if the form were freed before the queue drains (form created+freed in one message cycle), the block would later run on a dangling Self. }
...
begin
  ...
  { Set the main-form caption here (not in the synchronous Loaded body): by the time this queued block runs, RealCreateForms has returned and Application.MainForm is assigned, so the Self=Application.MainForm test is meaningful. }
...
{ Empty by default. Override in a descendant form to do startup work that should run after the window is on-screen (see the declaration comment + Loaded). }
...
begin
  ...
  { A secondary shown non-modally (AppData.ShowModal -> Show) may not be Screen.ActiveForm on some devices (observed: Samsung Tab S11), so FMX routes hardware Back's default Close to the MAIN form.
    TCommonCustomForm.Close (in FMX.Forms.pas) then calls Application.Terminate on the main form, tearing the app down while the secondary is still open.
    Close only terminates if CloseQuery returns True, so: if the main form is asked to close while a secondary TLightForm is open, close that secondary and VETO the close.
    Independent of which form FMX deems active. }
  if (Self = Application.MainForm)
...
{ This code is guaranteed to be called ONLY once.
  Tech details: It is enough to put SaveBeforeExit in these two places only: OnCloseQuery and OnDestroy.
...
begin
  { Catches the Android non-blocking-Show antipattern: caller did `AppData.ShowModal(F); FreeAndNil(F)` — but on Android ShowModal falls back to non-blocking Show, so FreeAndNil runs before the user sees the form.
    Use `Action:= caFree` in OnClose, or wire an AfterClose handler instead.

    Exempted cases (not the antipattern):
      - FEmbedded — embedded forms (Container reparented into a host); host manages lifetime, these are Free'd without Close by design. Most LearnAssist forms default to Visible=TRUE in their .fmx, so AfterConstruction Shows them before EmbedIn — Visible stays TRUE.
      - Released in FormState — legitimate caFree close: FMX's ReleaseForm sets this flag and queues Free, but DOES NOT set FVisible:=False (only caHide does). So Visible stays TRUE across the caFree path. Verified in ReleaseForm vs Hide, both in FMX.Forms.pas.
      ...
      - GAppShuttingDown — set on WillTerminate / TApplicationTerminatingMessage. Covers the window where DoneApplication->DestroyComponents frees a still-visible secondary during Halt, when none of Terminated / Terminating / csDestroying(Application) is set yet (see SHUTDOWN SENTINEL).
      - MainForm — always Visible at app shutdown.
    Clause order: cheapest checks first (short-circuit). NIL/destroying guards MUST come before any Application.* deref (Pascal short-circuits left-to-right). }
  Assert(NOT Visible
      OR GAppShuttingDown
      OR FEmbedded
...
{ Handles Escape key to close form when CloseOnEscape is True.
  Note: To use this, connect it to the form's OnKeyDown event in the FMX designer or in code: Self.OnKeyDown := FormKeyPress; }
  ...
  if Key = vkHardwareBack then
    begin
      ...
      if Self = Application.MainForm then
        begin
          { A secondary form shown non-modally (AppData.ShowModal -> Show on Android) sits on top of the main form, but FMX may deliver Back to the main form (Screen.ActiveForm).
            Backgrounding here would leave that form open underneath.
            Close the top-most secondary first; only background when the main form is truly alone. }
...
{ Adjusts form padding when the virtual keyboard appears/disappears.
  ...
  Only standalone (non-embedded) forms adjust — embedded forms get the adjustment for free through the layout cascade (parent form's padding shrinks all Client-aligned children).
  The pre-keyboard Padding.Bottom (e.g. the Android nav-bar inset set by ApplyAndroidWindowInsets) is captured on show and RESTORED on hide — zeroing it would clobber the inset. }
...
{ Override this method if you want to do your own loading from INI file.
  In this case, don't call inherited!
...
{-------------------------------------------------------------------------------------------------------------
   OTHERS
-------------------------------------------------------------------------------------------------------------}
{ Stretches the form vertically across the current monitor's work area (excludes taskbar). On mobile the form is fullscreen anyway, so the call is a no-op. }
...
{ One app-wide listener that FMX calls whenever the activity's decor-view insets change.
  ...
  Android does not publish window insets until the first onApplyWindowInsets dispatch, which the framework runs after onResume + the first layout traversal — strictly AFTER Pascal form construction.
  So on a cold start the main form's AfterConstruction sees isWindowInsetsDefined=FALSE and cannot set its safe-area padding yet; the toolbar would draw under the status bar until something re-triggered the apply.
  The number of message-loop pumps until that first dispatch is device-dependent (observed: 1 on a OnePlus Nord, MORE on a Samsung Tab S11 Ultra — on the Tab the toolbar overlapped the status bar on first show and only corrected after a manual minimize→restore, which re-fired BecameActive).
  Polling a fixed number of times is therefore guesswork.

  THE CLEAN FIX: subscribe to the callback FMX ALREADY fires from inside its own decor-view listener.
  FMXNativeActivity.onApplyWindowInsets calls onActivityInsetsChangedListener.insetsChanged(...) on every inset change (FMXNativeActivity.java).
  We register one TInsetsChangedListener via MainActivity.setOnActivityInsetsChangedListener; FMX invokes insetsChanged the instant the first dispatch lands (and again on rotation / multi-window resize), and we re-apply padding to every live TLightForm.
  Event-driven, no polling, no stolen listener slot (FMX keeps its own decor-view listener; we hook the callback it exposes).

  Single-slot discipline: setOnActivityInsetsChangedListener has ONE slot on the singleton activity, so exactly ONE listener is registered process-wide (guarded by GInsetsListener), not one per form.
  The callback walks Screen.Forms so all TLightForm instances — main + any visible secondary — get re-applied. }
TYPE
...
VAR I: Integer;
begin
  { FMX calls this on the UI thread from its decor-view onApplyWindowInsets. Re-apply to every self-saving form; ApplyAndroidWindowInsets is cheap and SameValue-guards against needless re-layout.
...
{ Registers the single app-wide insets listener on first call; later calls are no-ops.
  Called from every TLightForm.AfterConstruction, but only the first one that runs actually registers (GInsetsListener guard).
  See TInsetsChangedListener for the full rationale. }
...
{$IFDEF ANDROID}
{ Closes the top-most visible secondary TLightForm (not the main form, not an embedded form) and returns TRUE if one was closed.
  Screen.AddForm inserts new forms at index 0 (FMX.Forms.pas), so scanning index 0 upward visits the most-recently-shown (top-most) form first — for the nested Library Manager -> Edit Book case this closes only Edit Book, leaving Library Manager open.
  EXIT the moment one is closed: Close may free the form and mutate Screen.Forms, so we must not keep indexing into it.
  Embedded forms are skipped — their Container is reparented into a host, and closing one would orphan that host.
  A form already mid-Free (Released in FormState) is also skipped: ReleaseForm does NOT clear FVisible, so a released-but-not-yet-freed form is still Visible and would otherwise shadow a real secondary behind it (and re-Close returns caNone). }
...
{ Android only — reserve the system status-bar (top) and navigation-bar (bottom) safe-area as form Padding, but ONLY when the window is actually drawing edge-to-edge.
  See the declaration comment in the INTERFACE section for the full story.

  WHEN this padding is needed (and when it must NOT be applied):
  Edge-to-edge enforcement happens iff the device runs Android 15+ (SDK_INT >= 35) AND the app targets SDK >= 35 (Orinoco targets 36).
  Under enforcement the form is drawn at y=0, behind the opaque-but-transparent system bars, so the bar height has to be re-added as Padding or the toolbar sits under the clock/battery.
  On Android 14 and older the OS does the opposite: it keeps the bars opaque and lays the content out BELOW them already ("System bars remain opaque with reserved space below them" — Android 15 behavior-changes doc).
  On those devices the content is ALREADY inset, so adding Padding here double-counts the status bar and leaves an empty gap above the toolbar (the OnePlus-Nord regression, 2026-05-29..06-03).
  The fix is therefore not a different inset *value* — it is to apply the padding only when edge-to-edge is in force, and to zero it otherwise.

  WHY the inset value can't be trusted to self-zero: FMXNativeActivity installs an OnApplyWindowInsetsListener and reads getSystemWindowInsetTop() BEFORE the decor view consumes the insets, so it captures the real bar height even on a legacy below-the-bars window (per AOSP, the deprecated getter returns the height until the insets are consumed).
  Hence MainActivity.getWindowInsets reports a non-zero top on the Nord too — the only reliable discriminator is the OS version gate below.

  Mechanics: reads MainActivity.getWindowInsets (physical px), converts to DP via IFMXScreenService.GetScreenScale, assigns Padding.Top/Bottom.
  Padding cascades to every Align=Top/Bottom/Client child, so content reflows inside the visible window.
  SameValue skip avoids needless re-layout.
  Early-exit on isWindowInsetsDefined=False: on a cold start AfterConstruction lands before the first onApplyWindowInsets dispatch — the app-wide TInsetsChangedListener (see RegisterInsetsListener) re-fires this method the moment FMX publishes the insets, and again on every later change (rotation / multi-window). }
...
begin
  {$IFDEF ANDROID}
  { # Edge-to-edge gate }
  { Pre-Android-15 (or any non-edge-to-edge window): the OS already reserves space below the bars, so any Padding we add here is double-counting. Reset to zero and leave. }
