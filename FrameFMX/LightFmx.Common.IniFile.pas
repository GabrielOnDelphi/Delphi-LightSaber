...
--------------------------------------------------------------------------------------------------------------
  ...
  Storing only individual controls:
     ...
     Example:
        To store the status of a checkbox to an INI file use TIniFileApp.Write(MyCheckBox).
  ...
  Completeness:
     Most common FMX controls are supported.
  ...
  OnClick execution:
     ...
     In other words, if you want to auto-execute some OnClick event, you
  ...
  Notices
    * DO NOT USE SELF as parameter in TIniFileFmx.Create if you call TIniFileFmx.Create in FormCreate because Self is not yet ready! But TIniFileFmx.Create(FrmMain) works.
  ...
  Demo code:
     ...
     begin
       var INI := LightFmx.Common.IniFile.TIniFileApp.Create('SectionName');
       TRY
         INI.Write('LastID', 0);
       FINALLY
         FreeAndNil(INI);
...
{ToDo 2: Prevent controls from firing the OnClick event (and others like this) when loading state from INI. Do it like so: set control's event handler to NIL, load its value from INI file, then set the event handler back to original value
       How can I check a checkbox without triggering the OnClick event?
...
begin
  Assert(Form <> NIL);
  Assert(AutoState <> asNone, 'AutoState = asNone detected in SaveForm');
...
begin
  Assert(Form <> NIL);
  Assert(AutoState <> asNone, 'AutoState = asNone detected in LoadForm');
  ...
  if Ctrl.InheritsFrom(TForm) then
    begin
      ...
      { Non-resizable form?
        ...
        Note: SizeToolWin is sizable despite its name containing "Tool", so it's not in this list. }
      IsNonResizable :=
...
begin
  ...
  begin
    ...
    begin
      ...
      if Comp is TScrollBar      then TScrollBar(Comp).Value       := ReadFloat(Comp.Owner.Name, Comp.Name, 0) else
...
begin
  ...
  Assert(Font <> NIL, '[TIniFileApp.Read] Font cannot be nil');
...
begin
  Assert(Ident <> '', '[TIniFileApp.Write] Ident cannot be empty');
  Assert(Font <> NIL, '[TIniFileApp.Write] Font cannot be nil');

  WriteString (FSection, Ident,           '');  { Marker so the font can be found by identifier. Otherwise filtered out by TIniFileVCL.Read: if ValueExists(FSection, Comp.Name) }
